...
def run_realtime_inference():
    ...
    weights_path = os.path.abspath(os.path.join(src_dir, "..", "runs", "detect", "asl_yolov8_model", "weights", "best.pt"))
    
    if not os.path.exists(weights_path):
        print(f"[!] Error: Could not locate training weights at: {weights_path}")
        print("[!] Please run training (`python src/train.py`) first.")
        return
    ...
    while True:
        ...
        cv2.putText(annotated_frame, "System Mode: Live Prediction", (15, 30),
    ...
    print("[+] Video stream closed down successfully.")
